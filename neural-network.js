...
(function() {
  ...
  function updateNeural(s) {
    ...
    if (frame % 2 === 0 && s.signals.length < 60 && s.connections.length > 0) {
      var numSpawn = 2 + Math.floor(Math.random() * 4);
      for (var sp = 0; sp < numSpawn; sp++) {
        var c = s.connections[Math.floor(Math.random() * s.connections.length)];
        s.signals.push({ conn: c, t: 0, speed: 0.003 + Math.random() * 0.005, bright: Math.random() > 0.5 });
      }
    }
